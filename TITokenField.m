...
- (instancetype)initWithTitle:(NSString *)aTitle representedObject:(id)object font:(UIFont *)aFont {
    ...
    if ((self = [super init])){
        ...
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[_textLabel]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_textLabel)]];
...
{
    self.textLabel.textColor = self.selected ? self.tintColor : self.textColor;
}
...
- (void)setTitle:(NSString *)newTitle {
    ...
    if (newTitle){
        _title = [newTitle stringByAppendingString:@","];
